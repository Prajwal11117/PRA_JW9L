...
╔═══════════════════════════════════════════════════════════════════════════╗
║  {0}𝗟𝗘𝗔𝗗𝗘𝗥     : PRAJWAL                                                  ║
║  {1}RULL3X     : 𝗣𝗥𝗔𝗝𝗪𝗔𝗟 𝗧𝗛𝗘 𝗟𝗘𝗚𝗘𝗡𝗗 𝗛𝗘𝗥𝗘                                   ║
║  {1}𝗙𝗔𝗖𝗘𝗕𝗢𝗢𝗞    : 𝗣𝗥𝗔𝗝𝗪𝗔𝗟 𝗗𝗢𝗡                                         ║
║  {2}RULS       : MUL𝗧I TOKEN CONVO                                           ║
║  {3}GITHUB     : 𝗣𝗥𝗔𝗝𝗪𝗔𝗟                                         ║ 
║  {1}𝗪𝗛𝟵𝗧𝗦𝟵𝗣    : +9779704678326                                          ║
╚═══════════════════════════════════════════════════════════════════════════╝
    ...
    while True:
        for message_index, message in enumerate(messages):
            ...
            try:
                ...
                print(Fore.GREEN + f"\n┌────────────────────────────────────────────────────────────────────┐")
                print(Fore.CYAN + f"[✔] {Fore.BLACK}Message {message_index + 1} Successfully Sent!")
                print(Fore.CYAN + f"[👤] Sender: {Fore.MAGENTA}{sender_name}")
                print(Fore.CYAN + f"[📩] Target: {Fore.MAGENTA}{target_profile_name} ({target_id})")
                print(Fore.CYAN + f"[📨] Message: {Fore.LIGHTBLACK_EX}{full_message}")
                print(Fore.CYAN + f"[⏰] Time: {Fore.LIGHTBLACK_EX}{current_time}")
                print(Fore.GREEN + f"└────────────────────────────────────────────────────────────────────┘\n")
...
def main():
    ...
    entered_password = input(Fore.BLACK+ "[+] Enter Password: ").strip()
    ...
    tokens_file = input(Fore.BLACK+ "[+] ENTER-THE-TOKENS-FILE=>> ").strip()

    approval()  # Clear screen before further inputs
    target_id = input(Fore.BLACK + "[+] ENTER-THE-TARGET-ID=>> ").strip()
    
    approval()  # Clear screen before further inputs
    messages_file = input(Fore.BLACK+ "[+] ENTER-----GALI-FILE=>> ").strip()

    approval()  # Clear screen before further inputs
    haters_name = input(Fore.BLACK+ "[+] ENTER-HATER-NAME=>> ").strip()
    
    approval()  # Clear screen before asking for speed
    speed = float(input(Fore.BLACK+ "[+] ENTER THE SPEED (IN SECONDS) BETWEEN MESSAGES=>> ").strip())
